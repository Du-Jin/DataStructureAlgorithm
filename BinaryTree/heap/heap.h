...
class Heap {
public:
  bool MaxHeapInsert(ElementType data);
  ElementType MaxHeapDelete();
...
template <typename ElementType>
bool Heap<ElementType>::MaxHeapInsert(ElementType data) {
  // 将元素插入到堆最后位置
  head_[size_] = data;
  size_++;
...
template <typename ElementType>
ElementType Heap<ElementType>::MaxHeapDelete() {
  if(size_ == 0)
    return ElementType();
  ...
  if(size_ == 0)    // 删除结点后只有一个结点，则不需要调整
    return root_element;
  // 从上到下依次比较父节点和左右节点中最大的那个，迭代调整使满足堆结构要求
  for(int parent_index = 0, child_index = 1;
    ...
    // 选出左右子节点中大的那个，以进行比较，判断是否需要调整以满足堆的结构特性
    if ((child_index + 1 < size_) &&                    // 右子节点存在，且
        (head_[child_index+1] > head_[child_index]))    // 右字节点比左节点大
      child_index++;
    if(head_[parent_index] > head_[child_index])  // 已满足堆结构要求，不需要继续调整
      break;
    else {
...
template <typename ElementType>
Heap<ElementType>* MaxHeapBuild(ElementType *data, int size) {
  // 首先构造完全二叉树
  ...
  for(int parent = (size-2)/2, child = 2*parent+1;
    ...
    // 从上到下依次比较当前子树中父节点和左右子结点中最大的那个，迭代调整使满足堆要求
    for (int parent_index = parent, child_index = child;
      ...
      // 选出左右子节点中大的那个
      if((child_index + 1 < size) &&      // 右子节点存在且右子节点比左子节点大
